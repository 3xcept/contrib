...
```console
go get -u github.com/3xcept/contrib/entproto
```

Install `protoc-gen-entgrpc` (ent's gRPC service implementation generator):

```console
go get github.com/3xcept/contrib/entproto/cmd/protoc-gen-entgrpc
```
...
```console
go run github.com/3xcept/contrib/entproto/cmd/entproto -path ./ent/schema
```
...
option go_package = "github.com/3xcept/contrib/entproto/internal/todo/ent/proto/entpb";
...
go generate ./cmd/protoc-gen-entgrpc/... &&
  go install github.com/3xcept/contrib/entproto/cmd/protoc-gen-entgrpc &&
  go generate ./... &&
  go test ./...
```
